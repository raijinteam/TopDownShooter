...
public class NavigationMenuAnimation : MonoBehaviour
{
    ...
    public void OnClick_MenuActivate(int index)
    {
        ...
        var checkStatePlayerSelectInTutorial = UiManager.instance.ui_tutorial.tutorialState == TutorialState.OpenPlayerSelect;
        var checkPlayerSelectIndex = index == 1;
        ...
        if (UiManager.instance.CanChangeMenus)
        {
            for (int i = 0; i < all_MenusBG.Length; i++)
            {
                ...
                if (!DataManager.instance.isTutorialPlaying || (checkStatePlayerSelectInTutorial && checkPlayerSelectIndex))
                {
                    //I IS EQULS TO IDEX INCREASE SIZE OF BUTTON AND SET ACTIVE THAT BUTTON
                    if (i == index)
                    {

                        if (DataManager.instance.isTutorialPlaying && index == 1)
                        {
                            UiManager.instance.ui_tutorial.tutorialState = TutorialState.OpenAllPlayer;
